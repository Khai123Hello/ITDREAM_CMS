...
const targetConfig = configEndpoints.find(c => c.module === 'blog' && c.endpoint === 'educatorList');
const targetBe = beEndpoints.find(b => b.controller === 'BlogController.java' && b.path.includes('educator-list'));

console.log("Config target:", targetConfig);
console.log("BE target:", targetBe);

if (targetConfig && targetBe) {
    const normConfig = cleanPath(targetConfig.path.replace(/\/:[a-zA-Z0-9_]+/g, '/{id}'));
    const normBe = cleanPath(targetBe.path.replace(/\{[a-zA-Z0-9_]+\}/g, '{id}'));
    console.log("normConfig:", JSON.stringify(normConfig));
    console.log("normBe:", JSON.stringify(normBe));
    console.log("Path match?", normConfig === normBe);
    console.log("Method match?", targetConfig.method === targetBe.method);
}
